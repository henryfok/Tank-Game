...
public class GameBoard {
	...
	public GameBoard() {
		for (int row = 0; row < NUMBER_ROWS; row++) {
			for (int col = 0; col < NUMBER_COLS; col++) {
				board[row][col] = new CellState(false, false);
			}
		}
	}
	...
	public void recordTankInCell(CellLocation cell) {
		assert cellOpenForTank(cell);

		int row = cell.getRowIndex();
		int col = cell.getColIndex();
		board[row][col] = board[row][col].makeContainTank(); 
	}
}
